function [ft,ft2,wo_len,zt, zpt, x, r, z, P, wo, dw, wf] = Initialization(simtime1, simtime_len_train, simtime_len_test, simtime2,dt, nsecs, N, alpha);

%% Periodic 
amp = 1;
freq = 1/60;
ft = (amp/1.0)*sin(1.0*pi*freq*simtime1) + ...
    (amp/2.0)*sin(2.0*pi*freq*simtime1) + ...
    (amp/6.0)*sin(3.0*pi*freq*simtime1) + ...
    (amp/3.0)*sin(4.0*pi*freq*simtime1);
scale = max(ft);
ft = ft/ (1 * scale);
...
% end
%%
wo_len = zeros(1,simtime_len_train);
zt = zeros(1,simtime_len_train);
zpt = zeros(1,simtime_len_test);
x0 = 0.5*randn(N,1);
